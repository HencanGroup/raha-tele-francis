...
class MemberSeeder extends Seeder
{
    ...
    /**
     * Upserts one member user from a full JSON record, together with the
     * linked Member credit wallet. If the user already exists, all fields
     * are updated to reflect the latest values from the JSON file.
     *
    ...
    protected function createMember(array $record, $counties, $towns): void
    {
        ...
        if ($user->wasRecentlyCreated) {
            ...
            Log::info('MemberSeeder: updated member', ['email' => $email]);
            $this->command->info("  ↻ Updated member → {$user->email}");
        }
    }
}
